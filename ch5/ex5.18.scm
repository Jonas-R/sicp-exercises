	...
	(newline))
      ...
      (define (execute)
		...
		(set! instruction-count (+ instruction-count 1))
                ...
                (execute)))))
      (define (trace-reg-on regname)
	(let ((reg (assoc regname register-table)))
	  (if reg
	      (begin ((cadr reg) 'trace-on)
		     'done)
	      (error "Unknown register:" regname))))
      (define (trace-reg-off regname)
	(let ((reg (assoc regname register-table)))
	  (if reg
	      (begin ((cadr reg) 'trace-off)
		     'done)
	      (error "Unknown register:" regname))))
...
((fact-machine 'trace-reg-off) 'val)
